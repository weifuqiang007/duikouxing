...
### 6.2 画质时序稳定性调优记录（2026-08-26，wlh-007/008/009 三代对照）
...
4. 人脸部 ±1 亮度的"曝光闪"与鼻唇高频抖动是 HeyGem 逐帧重绘（无时序模型）的
   架构性残差；`stabilize.py` 的 EMA 色调校正（死区+限幅+细色度量化编码）可将其
   略微削峰但无法根除。若需根除：候选为脸部区光流几何稳定（约一天工作量）或
   阶段 D 中与 LatentSync 做 A/B 后降级 HeyGem 为实验引擎。
