...
| 13 | 进程在 analysing 100% 完成后、`processing 0%` 时**段错误退出**（bash 报 Segmentation fault，exit 139） | DirectML 偶发崩溃（驱动瞬时状态；实测 GPU 显存/温度正常、无竞争进程时也会发生） | **原样重跑一次即可**（见 8.11）；连续两次才需要排查 |
...
## 8.11 第二批量产：person1/2/3 各一条新视频（2026-08-30）

> 输入：`Desktop\视频\new20260828\person{1,2,3}\`，三条**新拍视频**（同日 17:35 放入，源脸 zhenglian/celian 与 8.28 归档字节级相同）
> 配置：全部沿用 #5（inswapper_128@128 + 正侧脸多源 + occlusion/region + blur 0.6 + expression_restorer 80）
> 三条全是 iPhone HDR（HEVC 10-bit HLG + rotation=-90），全走 8.9 配方转码，一次通过

| 任务 | 视频规格 | 帧数 | 处理耗时(fps) | 输出 | 差分验证(脸区/背景) |
|------|---------|------|--------------|------|-------------------|
| fs-p1-128-0002 | 42.8s | 1283 | 3:48 (5.6) | swap_p1_128.mp4 309MB | 3.8~4.4 / 1.8~2.3 ✅ |
| fs-p2-128-0002 | 38.0s | 1139 | 3:19 (5.7) | swap_p2_128.mp4 240MB | 3.7~3.8 / 1.8~2.0 ✅ |
| fs-p3-128-0002 | 31.5s | 945 | 2:52 (5.5) | swap_p3_128.mp4 177MB | 3.5~4.0 / 1.8 ✅ |

**本批新经验：**

1. **坑13（速查表 #13）**：person1 首跑在 analysing 完成后、`processing 0%` 处 DirectML 段错误（exit 139）。GPU 状态核查无异常（显存 3.3/12.3GB、36°C、无竞争进程），**原样重跑即成功**，person2/3 未复现——判定为驱动瞬时状态，量产脚本遇 139 直接重试一次即可。
2. **低饱和场景的判定方法**：person2 转码后饱和度仅 16~17（低于灰雾阈值 20），但对比度 56~60 正常——先抽**原始 HDR 帧**测同指标（实测原片 12~13，转码后反而更高），确认是"白墙+白证件"场景本身素净，而非转码洗色。**饱和度低先对比原片，别急着调配方**。
3. **内容与验收备注**：三条均为单人正脸举证件场景（身份证+特种作业操作证），`--face-selector-mode one` 命中真人脸（远大于证件照小脸）；FaceFusion 输出音频为 flac 重编码，规格完整保留（1080×1920/30fps 全帧数）。
4. 目视对比图（左原右换）在各任务 `work\side_*.png`，交付前可快速翻查。

## 8.12 用户反馈"头发发糊/死黑"归因 + 验收结论（2026-08-30，person3）

**业务结论（用户验收）**：FaceFusion 管线**基本满足客户诉求，但仍需提升**。头发观感问题是当前主要短板。

**症状**：成品里头发部分有的区域死黑、有的区域发虚糊成片。用户质疑：FaceFusion 不是只换脸吗，头发为什么变了？

**排查结论：换脸没动头发，观感变化来自 HDR→SDR 转码 + 二次有损编码。** 客观证据（帧 n=450）：

| 排查项 | 方法 | 结果 |
|--------|------|------|
| 换脸动没动头发 | 输入输出差分热力图（3帧平均） | 高变化区严格限于椭圆脸区五官；头顶/鬓角≈0；发际线只有一条窄羽化带 |
| 静态管线复跑 | 同配置跑单帧，发区对比原帧 | 发区差分 0.1~0.7（没动），脸区 6.8~11.8（换了） |
| 换 256 模型 | ghost_2_256 vs inswapper_128 | 发区差异 0.0，对头发无效 |
| box∩ 收边 | mask 加 box + padding 12 | 与基线差 0.07%，裁剪框有外边距，小 padding 碰不到发际线 |
| 单源侧脸 | 仅 celian 跑静态 | 发区同样没动，排除多源切换嫌疑 |
| 暗部纹理逐阶段 | 发区灰度<80 的 std | HDR参考 25.1 → SDR中间件 21.0 → 成品 20.4。**转码丢 4 分，换脸只丢 0.6** |
| 转码配方变体 | 去 eq / crf10+aq3 / 10-bit | darkstd 20.2→20.9→21.1，收益 ~4%；10-bit 中间件 FaceFusion 不认（坑1），不可用 |

**代码级发现（修正 8.5 命令注释的误解）**：读 `facefusion/face_masker.py` + `face_swapper/core.py` 源码：
- `--face-mask-blur` / `--face-mask-padding` **只作用于 box 遮罩**；occlusion/region 遮罩的羽化在代码里写死 Gaussian(5)
- padding 取值 0~100 无负值，且从裁剪框边缘收缩——框顶在头发上方的背景里，小 padding 到不了发际线
- 多 mask 类型是 `numpy.minimum.reduce` **取交集**，occlusion+region 已是最紧组合，去掉任何一个只会更大

**提升方向**（按投入排序）：
1. 接受现状继续量产——发际线窄羽化带 + 8-bit 暗部落差是当前管线天花板，客户已基本接受
2. 成品交付前重编码 crf12 + aq-mode3，略减暗部块状感（~4% 收益，低成本可做）
3. **整头生成（LivePortrait 方案A）**——同时消灭贴脸边界和转码暗部损失，是结构性提升，沿用既有实验计划推进

排查图集：`jobs-home/fs-p3-128-0002/work/`（heatmap_head.png / stage3_hair.png / recipe4.png / ab_*）。
